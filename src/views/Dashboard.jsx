    ...
    <div className="pb-24 md:pb-6">
      ...
      <div className="pt-8 pb-6 px-6">
        <div className="flex items-center gap-3 mb-6">
          <button 
            onClick={onBack}
            className="p-2 rounded-full glass-panel hover:bg-black/5 dark:hover:bg-white/10 transition text-[var(--text-muted)] hover:text-[var(--text-main)] flex-shrink-0"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[var(--accent-blue)] to-[var(--accent-purple)] flex-1 truncate">
            {data.metadata.title}
          </h1>
          <button className="p-2 rounded-full glass-panel hover:bg-black/5 dark:hover:bg-white/10 transition flex-shrink-0">
        ...
        >
          ...
          >
            ...
            <div className="text-xs text-[var(--text-muted)]">Total Trip Expenses</div>
          </div>
        </motion.div>
      </div>
      ...
      <div className="px-6">
        <h3 className="text-lg font-semibold mb-4 text-[var(--text-main)]">Trip Crew</h3>
        ...
        <form onSubmit={addParticipant} className="flex gap-2 mb-6">
          <input
            ...
            className="flex-1 bg-[var(--panel-bg)] border border-[var(--glass-border)] rounded-xl px-4 py-3 text-[var(--text-main)] placeholder-slate-400 focus:outline-none focus:border-[var(--accent-blue)] focus:ring-1 focus:ring-[var(--accent-blue)] transition-all"
          />
          <button 
            type="submit"
            disabled={!newParticipant.trim()}
            className="bg-gradient-to-r from-[var(--accent-blue)] to-[var(--accent-purple)] text-white p-3 rounded-xl font-medium shadow-lg shadow-[var(--accent-blue)]/20 disabled:opacity-50 disabled:shadow-none transition-all flex items-center justify-center"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            {data.participants.map((participant) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-[var(--text-main)]">{participant.name}</span>
                </div>
                <button 
          ...
          {data.participants.length === 0 && (
            <div className="col-span-full text-center py-8 text-[var(--text-muted)] border border-dashed border-[var(--glass-border)] rounded-xl">
